...
# your code here
valid_tweets = raw_data.map(safe_parse) \
...
# your code here
print_users_count(valid_tweets.count())
...
group_tweets = valid_tweets.map(lambda x: (get_group_id(x[0]), x[1])) \
                           .partitionBy(7).cache()
group_rdd = group_tweets.map(lambda x: (x[0], len(x[1]))) \
...
# def reduce_by_pop(tokens):
#     pop_dict = {}
#     for token in tokens:
#         token_str = token[1]
#         if pop_dict.has_key(token_str):
#             pop_dict[token_str] = pop_dict[token_str] + 1
#         else:
#             pop_dict[token_str] = 1
#
#     # update rel_popularity
#     for token in pop_dict:
#         pop_dict[token] = get_rel_popularity(pop_dict[token], valid_tokens_count[token])
#     # sort by popularity
#     sorted_pop_dict = sorted(pop_dict.items(), key = lambda x: (-x[1], x[0]))
#     return sorted_pop_dict[:10]
#
# start_time = time.time()
# group_tokens = user_tokens.filter(lambda t : valid_tokens_count.has_key(t[1])) \
#                           .map(lambda t : (get_group_id(t[0]), t[1])) \
#                           .groupBy(lambda t: t[0], 7) \
#                           .sortByKey() \
#                           .map(lambda t: reduce_by_pop(list(t[1])))
#
# group_tokens_col = group_tokens.collect()
# for i in range(8):
#     print_tokens(group_tokens_col[i], i)
# print("--- Get group result: %s seconds ---" % (time.time() - start_time))
