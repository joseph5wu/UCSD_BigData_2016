...
# files = sc.textFile('../Data/hw2-files.txt').collect()
...
def safe_parse(raw_json):
    ...
    # check whether contains text and user_id
    if('text' in json_object and 'user' in json_object and 'id_str' in json_object['user']):
        return (json_object['user']['id_str'].encode('utf-8'), json_object['text'].encode('utf-8'))
    else:
        return None

# your code here
start_time = time.time()
valid_tweets = raw_data.map(safe_parse).filter(lambda tweet: tweet is not None).cache()
# print valid_tweets.count()
# print valid_tweets.take(2)
print("--- Prepare valid tweets: %s seconds ---" % (time.time() - start_time))
...
# your code here
start_time = time.time()
users = valid_tweets.map(lambda x:x[0]).distinct()
print_users_count(users.count())
print("--- Get user count: %s seconds ---" % (time.time() - start_time))
...
import pickle
start_time = time.time()
pickle_file = open('../Data/users-partition.pickle', 'rb')
partition = pickle.load(pickle_file)
print("--- Read partition file: %s seconds ---" % (time.time() - start_time))
...
def get_group_id(user_id):
    if(partition.has_key(user_id)):
        return partition[user_id]
    else:
        return 7

# valid_tweets = valid_tweets.partitionBy(numPartitions = 7, partitionFunc = lambda x: get_group_id(x))
group_rdd = valid_tweets.map(lambda tweets:(get_group_id(tweets[0]), 1)) \
...
print("--- Partition-user count: %s seconds ---" % (time.time() - start_time))
...
# your code here
def print_tokens_count(count):
    print 'Number of elements:', count

start_time = time.time()
tokens = valid_tweets.flatMap(lambda tweet:tok.tokenize(tweet[1])).distinct()
print_tokens_count(tokens.count())
print("--- Token count: %s seconds ---" % (time.time() - start_time))
...
# your code here
start_time = time.time()
user_tokens = valid_tweets.flatMapValues(lambda text:tok.tokenize(text)).distinct()
token_count = user_tokens.map(lambda t:(t[1], 1)) \
                         .reduceByKey(lambda x, y:x + y) \
                         .filter(lambda x:x[1] >= 100).cache()

print 'Number of elements:', token_count.count()

token_count_col = token_count.collect()
token_count_col.sort(key = lambda x:x[1], reverse = True)
print_tokens(token_count_col[:20])
print("--- Pop 20 token count: %s seconds ---" % (time.time() - start_time))
...
# your code here
start_time = time.time()
valid_tokens_count = token_count.collectAsMap()
print("--- Prepare valid tokens: %s seconds ---" % (time.time() - start_time))

start_time = time.time()
group_tokens = user_tokens.filter(lambda t : valid_tokens_count.has_key(t[1])) \
                          .map(lambda t : (get_group_id(t[0]), t[1])) \
                          .partitionBy(7).cache()
print("--- Prepare group: %s seconds ---" % (time.time() - start_time))

# print group_tokens.first()
# group_data = []
# start_time = time.time()
for i in range(0, 8):
    start_time = time.time()
    i_group_token = group_tokens.filter(lambda t : t[0] == i) \
                                .map(lambda t : (t[1], 1)) \
                                .reduceByKey(lambda x, y : x + y) \
                                .map(lambda t : (t[0], get_rel_popularity(t[1], valid_tokens_count[t[0]])))

    i_group_token_col = i_group_token.collect()
    i_group_token_col.sort(key = lambda x : x[1], reverse = True)
    # print_tokens(i_group_token.take(10), i)
    print_tokens(i_group_token_col[:10], i)
    # group_data.append(i_group_token_col[:10])
    print("--- Get group result: %s seconds ---" % (time.time() - start_time))

# print("--- Get group result: %s seconds ---" % (time.time() - start_time))
# print group_tokens.take(2)
