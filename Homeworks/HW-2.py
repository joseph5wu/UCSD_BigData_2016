...
# In[13]:
import time
# Your code here ../Data/hw2-files-final.txt
...
group_tweets = valid_tweets.map(lambda x: (get_group_id(x[0]), x[1])).cache()
...
# your code here
def remove_duplicate_token(tweets):
    tokens_set = set()
    for t in tweets:
        tokens_set.update(tok.tokenize(t))
    return tokens_set

# start_time = time.time()

tokens = group_tweets.flatMap(lambda x: remove_duplicate_token(x[1])).cache()
print_count(tokens.distinct())
...
token_count = tokens.map(lambda x: (x, 1)) \
                         .reduceByKey(lambda x, y: x + y) \
...
group_tokens = group_tweets.flatMapValues(lambda x: remove_duplicate_token(x)) \
                           .filter(lambda x: valid_tokens_count.has_key(x[1])) \
                           .cache()
...
for i in range(8):
    ...
    i_group_token_col.sort(key = lambda x: (-x[1], x[0]))
